...
const FIXED_COSTS = Array.from({ length: 50 }, (_, i) => ({
    id: i + 1,
    nombre: `Servicio Legal Estándar ${i + 1}`,
    tarifa: Math.floor(Math.random() * 450) + 50,
    tiempo: (Math.floor(Math.random() * 8) + 1) * 0.5,
}));
...
const TimeBillingMaestro: React.FC<{ onCancel?: () => void }> = ({ onCancel }) => {
  ...
  const [descriptionMode, setDescriptionMode] = useState<'text' | 'fixed'>('text');
  ...
  const handleDrop = async (e: React.DragEvent, date: string, hour: number) => {
    ...
    if (!entryId) return;
        ...
        setTimeEntries(prev => prev.map(entry => entry.id === entryId ? { ...entry, fecha_tarea: date, hora_inicio: `${String(hour).padStart(2, '0')}:00:00`, estado: 'pending' } : entry));
        try {
            ...
            if (error) throw error;
            alert("Su solicitud de cambio ha sido enviada al administrador para su aprobación.");
            await fetchWeekEntries(currentUserProfile);
                ...
                <div className="md:col-span-2 space-y-4">
                    <div className="flex gap-4 border-b border-zinc-900 pb-4">
                        <button 
                            ...
                            className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest transition-colors border ${descriptionMode === 'text' ? 'bg-white text-black border-white' : 'text-zinc-500 border-zinc-800 hover:text-white hover:border-zinc-500'}`}
                        >
                            DESCRIPCIÓN
                        </button>
                        <button 
                            type="button" 
                            onClick={() => setDescriptionMode('fixed')}
                            className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest transition-colors border ${descriptionMode === 'fixed' ? 'bg-white text-black border-white' : 'text-zinc-500 border-zinc-800 hover:text-white hover:border-zinc-500'}`}
                        >
                            COSTOS FIJOS
                        </button>
                    </div>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full bg-zinc-900 border border-zinc-800 p-3 text-[10px] font-bold tracking-widest uppercase focus:outline-none focus:border-zinc-500"
                                />
                            </div>
                            <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-zinc-800">
                                {FIXED_COSTS.filter(fc => fc.nombre.toLowerCase().includes(fixedCostSearch.toLowerCase())).map(fc => (
                                    <button 
                                        key={fc.id}
                                        type="button"
                                        onClick={() => {
                                            setTaskDescription(fc.nombre);
                                            setRate(fc.tarifa);
                                            setHoursWorked(fc.tiempo);
                                        }}
                                        className={`text-left p-3 text-[10px] font-bold tracking-widest uppercase border transition-colors ${taskDescription === fc.nombre ? 'bg-zinc-800 border-white text-white' : 'bg-black border-zinc-900 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300'}`}
                                    >
                                        {fc.nombre} - ${fc.tarifa} ({fc.tiempo}h)
                                    </button>
                                ))}
                            </div>
                        </div>
