...
const AdminChatView: React.FC<{ session: Session }> = ({ session }) => {
    ...
    const [activeTab, setActiveTab] = useState<'abogado' | 'estudiante' | 'cliente' | 'admin'>('abogado');
    ...
    useEffect(() => {
        const fetchChatData = async () => {
            ...
            if (profiles) setUsers(profiles.filter(p => p.id !== session.user.id));
    ...
    const filteredUsers = users.filter(u => {
        ...
        if (activeTab === 'admin') return u.rol === 'admin';
        ...
        <div className="animate-in fade-in duration-500 font-mono w-full max-w-6xl mx-auto h-[75vh] flex flex-col md:flex-row border border-zinc-800 bg-black">
            <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-zinc-800 flex flex-col bg-zinc-950">
                <div className="flex border-b border-zinc-800 flex-shrink-0">
                    ...
                    <button onClick={() => setActiveTab('admin')} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest transition-colors ${activeTab === 'admin' ? 'bg-zinc-900 text-white border-b-2 border-white' : 'text-zinc-500 hover:text-white hover:bg-zinc-900/50'}`}>Admins</button>
                </div>
                        ...
                        filteredUsers.map(user => (
                            <button key={user.id} onClick={() => handleContactClick(user)} className={`w-full text-left p-4 border-b border-zinc-800/50 flex items-center gap-4 transition-colors ${selectedContact?.id === user.id ? 'bg-zinc-900 border-l-2 border-l-white' : 'hover:bg-zinc-900 border-l-2 border-l-transparent'}`}>
                                ...
                                <div>
                                    ...
                                    <p className={`text-[9px] uppercase tracking-widest mt-1 ${activeTab === 'abogado' ? 'text-green-400' : activeTab === 'estudiante' ? 'text-blue-400' : activeTab === 'admin' ? 'text-red-400' : 'text-zinc-400'}`}>{activeTab === 'cliente' ? 'Cliente' : activeTab}</p>
                                </div>
                            </button>
                        ))
                    )}
...
const AdminDashboard: React.FC<{ session: Session }> = ({ session }) => {
    ...
    return (
        <div className="bg-black h-screen text-white flex flex-col font-sans relative overflow-hidden">
            <style>{`
            ...
            <header className="flex-shrink-0 flex justify-between items-center p-6 bg-black z-40 border-b border-zinc-900/50">
            ...
            <main className="flex-grow flex flex-col relative z-10 overflow-y-auto scrollbar-hide">
                {activeView === 'HOME' ? (
